...
class DataTableComponent extends React.Component {
  ...
  render() {
    ...
    let postsArr = [];
    for (
      let i = (currentPage - 1) * maxItems;
      i < currentPage * maxItems && i < posts.length;
      i += 1
    ) {
        ...
        <Table hover>
          <thead>
            <tr>
              {header.map((v, idx) => (
                <th key={idx}>
                  <div className="center-cell">{v}</div>
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {postsArr.map((v, idx) => (
              <tr
                ...
                className={onItemClick !== undefined ? "cell-hover" : ""}
              >
                <td>
                ...
                <td>
                  <div className="center-cell">
                    {moment(v.createdDate).format("DD/MM/YYYY HH:mm")}
                  </div>
                </td>
                <td>
                  <div className="center-cell">
                    {this.getCategory(v.category)}
                  </div>
                </td>
                <td>
                  <div className="center-cell">
                ...
                {type === "admin" ? (
                  <td>
                    {v.status === 0 ? (
                      <div className="center-cell">
                        <Image
                          src={ic_accept_post}
                          onClick={e => onAccept(e, v.id)}
                        />
                        <Image
                          src={ic_cancel_post}
                          onClick={e => onDeny(e, v.id)}
                        />
                      </div>
                    ) : (
                      <div className="center-cell">
                        <Image src={ic_accept_post_disable} disable />
                        <Image src={ic_cancel_post_disable} disable />
                      </div>
                    )}
                  </td>
                ) : (
                  <td>
                    {v.status === 1 ? (
                      <div className="center-cell">
                        <Image src={ic_edit_disable} disable />
                        <Image src={ic_trash_disable} disable />
                      </div>
                    ) : (
                      <div className="center-cell">
                        <Image src={ic_edit} onClick={e => onEdit(e, v.id)} />
                        <Image
                          src={ic_trash}
